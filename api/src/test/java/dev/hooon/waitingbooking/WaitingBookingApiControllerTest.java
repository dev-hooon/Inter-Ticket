...
import dev.hooon.common.entity.DatabaseCleaner;
...
@SpringBootTest
@ActiveProfiles("test")
@ComponentScan(basePackages = {"dev.hooon.waitingbooking", "dev.hooon.auth.application", "dev.hooon.common.entity"})
...
class WaitingBookingApiControllerTest extends ApiTestSupport {
	...
	void setUp() {
		...
		userService.join(new UserJoinRequest("hello@example.com", "hello-name", "hello-password"));
	}
	...
	void registerWaitingBookingTest() throws Exception {
		...
		ResultActions actions = mockMvc.perform(
			MockMvcRequestBuilders
				...
				.header("Authorization", accessToken) // Include JWT token in the request header
