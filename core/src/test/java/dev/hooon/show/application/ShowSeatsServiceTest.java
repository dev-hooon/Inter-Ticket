...
class ShowSeatsServiceTest {
	...
	void findShowSeatsByShowIdAndDateAndRound_test() {
		...
		List<SeatsInfoDto> expected = new ArrayList<>(seatsInfoDtoList);
		expected.get(0).setSeats(vipSeatsDetailDtoList);
		expected.get(1).setSeats(sSeatsDetailDtoList);
		expected.get(2).setSeats(aSeatsDetailDtoList);
